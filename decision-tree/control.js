    ...
    function displayTreePredictions() {
        ...
        for (var x = 0; x < canv.width; x++) {
            for (var y = 0; y < canv.height; y++) {
                ...
                var prediction = forest.predict({
                    x: x,
                    y: y
                });

                var sum = 0;
                for (var predictedHexColor in prediction) {
                    sum += prediction[predictedHexColor];
                }

                for (var predictedHexColor in prediction) {
                    var numberOfVotes = prediction[predictedHexColor];
                    putPixel(imageData, canv.width, x, y, predictedHexColor, numberOfVotes * MAX_ALPHA / sum);
                }
            }
        }
